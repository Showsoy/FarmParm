...
<div class="pageform">
	...
	<form method="post" name="qna">
			...
			<c:if test="${board.user_id eq id || id eq 'admin'}">
				<span style="padding:0 0 0 650px;"><button type="button" id="wbutton" onclick="goto_url('qnaRemove.bo?item_code=${board.code}&bnum=${board.board_num}&page=${page }<c:out value="${keyword !=null ? '&keyword=' : '' }"/>${keyword}')">삭제</button></span>
			</c:if>
			<br><br>
			...
			<section id="commandCell">
				<button type="button" onclick="location.href='qnaList.bo?page=${page}<c:out value="${keyword !=null ? '&keyword=' : '' }"/>${keyword}'">목록</button>
			</section>
		</form>
	</div>	
</div>
